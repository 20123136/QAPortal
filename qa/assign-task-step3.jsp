...
<head>
	...
	function cacheCondition() {
		...
		//缓存Step2的条件
		for(var i=0; i<tabStep2.SampleConditionFields.length; i++) {
			var key = tabStep2.SampleConditionFields[i].getKey();
			var val = tabStep2.SampleConditionFields[i].getVal();
			var rawval = tabStep2.SampleConditionFields[i].getRawVal();
			var txt = tabStep2.SampleConditionFields[i].getTxt();
	...
	function getTaskStatus() {
		timerId = window.setInterval(function() {
				Ext.Ajax.request({
					url: window.webRoot + 'QASample/Task/Status/' + tabStep0.getTaskNo(),
					method: 'GET',
...
//							Ext.Msg.alert('提交失败', respText.message + "（" + respText.code + "）");
							...
							if(respText.data.status == 'Begin') {
								...
								Ext.getDom('imgAssingTaskStatus').src = '../images/loading.gif';
							} else if(respText.data.status == 'doSample') {
								Ext.getDom('spanAssingTaskStatus').innerHTML = '抽样中';
								Ext.getDom('imgAssingTaskStatus').src = '../images/loading.gif';
							} else if(respText.data.status == 'doAssign') {
								Ext.getDom('spanAssingTaskStatus').innerHTML = '分配中';
								Ext.getDom('imgAssingTaskStatus').src = '../images/loading.gif';
								...
								if(respText.data.result == '000') {
									...
									Ext.getDom('spanAssignUserCount').innerHTML = respText.data.userCountPractice;
									Ext.getDom('spanAssingCount').innerHTML = respText.data.sampleCountPractice;
				...
				items: [
					{
						...
						html: '<table width=666 align=center border="0" cellpadding="0" cellspacing="5" width="100%">' +
								...
								'</tr>' +
								'<tr>' +
								'<td width=100 align=left>计划执行时间: </td>' +
								'<td colspan=3 align=left><span id=spanAssignTaskPlannedTime style="color:blue;"></span></td>' +
								'</tr>' +
							   '</table>'
					}
				]
				...
				items: [
					{
						...
						html: '<table width=666 align=center border="0" cellpadding="0" cellspacing="5" width="100%">' +
								...
								'<td width=345 align=left><span id=spanAssignTaskSpentTime style="color:blue;"></span></td>' +
								'</tr>' +
								'<tr id=trResultTotal>' +
								'<td width=100 align=left>抽样数量: </td>' +
								'<td width=135 align=left><span id=spanAssingCount style="color:blue;"></span></td>' +
								'<td width=100 align=left>分配人数: </td>' +
								'<td width=345 align=left><span id=spanAssignUserCount style="color:blue;"></span></td>' +
								'</tr>' +
								'<tr>' +
								'<td width=100 align=left>运行状态: </td>' +
								'<td colspan=3 align=left><span id=spanAssingTaskStatus style="color:blue;"></span>&nbsp;&nbsp;<img id=imgAssingTaskStatus src=../images/loading.gif style="margin-bottom:-3px"></td>' +
								'</tr>' +
							   '</table>'
					}
				]
					...
					listeners:
						{
							'click': function(obj, e) {
								...
								obj.targetTab.add({
									...
									autoScroll:true,
									autoWidth:true,
									html:'<iframe name=tabStep4 src="assign-task-detail-list.jsp?taskNo=' + tabStep0.getTaskNo() + '&logId=' + logId + '" border=0 width="100%" height="100%" frameborder="no" scrolling="auto"></iframe>'
								}).show();
							}
						}
				})
...
*/
		...
		Ext.Ajax.request({
			url: window.webRoot + 'QASample/Task/Manual',
			method: 'POST',
			...
			success: function(response,options) {
				...
				if(respText.code != 'OK') {
					...
					Ext.getDom('spanAssignTaskPlannedTime').innerHTML = tabStep0.getPlannedInfo();
