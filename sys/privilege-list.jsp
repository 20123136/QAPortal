...
<head>
	...
	Ext.onReady(function(){
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var node = tree.getSelectionModel().getSelectedNode();
						if(!node) {
							Ext.Msg.alert('删除权限', '请选择要删除的权限！');
						} else {
							Ext.Msg.show({
								title:'删除权限?',
								msg: '请再次确实是否要将选中的权限删除?',
								buttons: Ext.Msg.YESNOCANCEL,
								fn: function(btn, text) {
									if(btn == "yes") {
										Ext.Ajax.request({
											url: window.webRoot + 'Privilege/' + node.attributes.id,
											method: 'POST',
											params: {
												_method: 'DELETE'
											},
		...
		tree = new Ext.ux.tree.TreeGrid({
			...
			dataUrl: window.webRoot + 'Privilege/TreeView/0'
		});
		...
		tree.on('movenode', function(t, node, oldParent, newParent, index) {
			Ext.Ajax.request({
				...
				url: window.webRoot + 'Privilege/'+node.attributes.id+'/moveto/'+newParent.attributes.id,
				success: function() {
					
				},
				failure: function() {
					Ext.Msg.alert('保存失败', '移动权限的操作失败！');
					tree.getLoader().load(tree.getRootNode());
				}
			});
		});
			...
			buttons: [{
				...
				handler: function() {
					...
					if(form.form.isValid()) {
						...
						if(actiontype == "edit") {
							form.form.submit({
								url: window.webRoot + 'Privilege/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
								success: function(form, action) {
									Ext.Msg.alert('修改成功', '修改权限成功！');
									tree.getLoader().load(tree.getRootNode());
									win.hide();
								},
								failure: function(form, action) {
