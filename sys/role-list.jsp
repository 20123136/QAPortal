...
<head>
	...
	//菜单类权限
	var pri_menu_store = new Ext.data.JsonStore({
		fields: pri_fields
	});

	//功能类权限
	var pri_func_store = new Ext.data.JsonStore({
		fields: pri_fields
	});

	//数据类权限
	var pri_data_store = new Ext.data.JsonStore({
		fields: pri_fields
	});

	//菜单类权限
	var pri_menu_store_self = new Ext.data.JsonStore({
		fields: pri_fields
	});

	//功能类权限
	var pri_func_store_self = new Ext.data.JsonStore({
		fields: pri_fields
	});

	//数据类权限
	var pri_data_store_self = new Ext.data.JsonStore({
		fields: pri_fields
	});

	Ext.Ajax.request({
		url: window.webRoot + 'Privileges',
		success: function(response){
			var vals = Ext.util.JSON.decode(response.responseText);
			if(vals.code == 'OK'){
				var i = 100;
				Ext.each(vals.data, function(val){
					if(val.typeId == 'MENU') {
						pri_menu_store.add(new pri_menu_store.recordType(val, i++));
					} else if(val.typeId == 'FUNC') {
						pri_func_store.add(new pri_func_store.recordType(val, i++));
					} else if(val.typeId == 'DATA') {
						pri_data_store.add(new pri_data_store.recordType(val, i++));
					}
				});

				pri_menu_store.singleSort('order', 'ASC');
				pri_func_store.singleSort('fatherId', 'ASC');
			}
		}
	});
							...
							Ext.Msg.show({
								...
								fn: function(btn, text) {
									if(btn == "yes") {
										...
										Ext.Ajax.request({
											url: window.webRoot + 'Role/enable',
											method: 'POST',
											params: {
												_method: 'PUT',
												id: ids
							...
							Ext.Msg.show({
								...
								fn: function(btn, text) {
									if(btn == "yes") {
										...
										Ext.Ajax.request({
											url: window.webRoot + 'Role/disable',
											method: 'POST',
											params: {
												_method: 'PUT',
												id: ids
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						win_assign_user.setIconClass("groupbtn");
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						win_assign_pri.setIconClass("keybtn");
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('分配权限', '请选择要分配权限的角色！');
						} else {
							if(grid.getSelectionModel().getCount() == 1) {
								Ext.getCmp('tabPrivilege').reset();
								Ext.Ajax.request({
									url: window.webRoot + 'Role/' + rec.get('id') + '/Privileges',
									success: function(response){
										var ids = new Array();
...
*/
				...
				handler: function() {
					if(form.form.isValid()) {
						...
						if(actiontype == "edit") {
							...
							form.form.submit({
								url: window.webRoot + 'Role/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
			...
			listeners: 
				{'load': function(obj, recs, opts) {
						if(grid.getSelectionModel().getCount() == 1) {
							var rec = grid.getSelectionModel().getSelected();
							Ext.Ajax.request({
								url: window.webRoot + 'Role/' + rec.get('id') + '/users',
								success: function(response){
									var ids = new Array();
									var val = Ext.util.JSON.decode(response.responseText);
									if(val.code == 'OK'){
		...
		userFromStore.on('beforeload', function(obj) {
			obj.baseParams.start = 0;
			obj.baseParams.limit = 500;
		});
			...
			buttons: [
				{
					...
					handler: function() {
						...
						if(rec) {
							...
							Ext.Ajax.request({
								url: window.webRoot + 'Role/' + rec.get('id') + '/Assign/Users',
								params: {
									_method: 'PUT',
									userid: userids,
									id: ids
			...
			buttons: [
				{
					...
					handler: function() {
						var rec = grid.getSelectionModel().getSelected();
						if(rec) {
							var pids = Ext.getCmp('tabPrivilege').getVal();
							Ext.Ajax.request({
								url: window.webRoot + 'Role/' + rec.get('id') + '/Assign/Privileges',
								params: {
									_method: 'PUT',
									pid: pids
								},
