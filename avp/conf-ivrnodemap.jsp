	...
	Ext.onReady(function(){
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'Conf/IVRNodeMaps'
			})
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>IVR菜单节点映射页面用于维护新旧IVR菜单节点的对照表。'旧菜单编号'为现有IVR(IR)定义的IVR菜单节点编号，'新菜单编号'为新IVR(VP)定义的IVR菜单节点编号。</font>"
			})
		);
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						if(!rec) {
							Ext.Msg.alert('编辑菜单节点映射关系', '请选择要修改的菜单节点映射关系！');
						} else {
							actiontype = "edit";
							win.setTitle("编辑菜单节点映射关系");
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('删除菜单节点映射关系', '请选择要删除的菜单节点映射关系！');
						} else {
							Ext.Msg.show({
								title:'删除菜单节点映射关系',
								msg: '请再次确实是否要将选择的菜单节点映射关系删除?',
										...
										Ext.Ajax.request({
											...
											success: function(response,options) {
												...
												if(respText.code == 'OK') {
													...
													Ext.Msg.alert('删除失败', '删除菜单节点映射关系失败！');
												}
											},
											failure: function() {
												grid.el.unmask();
			...
			buttons: [{
				...
				handler: function() {
					if(form.form.isValid()) {
						if(actiontype == "add") {
							form.form.submit({
								url: window.webRoot + 'Conf/IVRNodeMap',
								method: 'POST',
								params: {
									_method: 'POST'
								},
								success: function(form, action) {
									Ext.Msg.alert('保存成功', '添加菜单节点映射关系成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('保存失败', '添加菜单节点映射关系失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
						...
						if(actiontype == "edit") {
							...
							form.form.submit({
								url: window.webRoot + 'Conf/IVRNodeMap/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
								success: function(form, action) {
									Ext.Msg.alert('修改成功', '修改菜单节点映射关系成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.CONNECT_FAILURE:
											...
											break;
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('修改失败', '修改菜单节点映射关系失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
