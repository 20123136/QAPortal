	...
	Ext.onReady(function(){
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'Confs/Type/BusinessType'
			})
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>本页面用于维护业务类型。'编号'为VP内部定义的业务编号，'名称'为业务类型名称。'中继号码'为该业务对应的进线号码列表, 多个号码以逗号分隔。</font>"
			})
		);
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						if(!rec) {
							Ext.Msg.alert('编辑业务类型', '请选择要修改的业务类型！');
						} else {
							actiontype = "edit";
							win.setTitle("编辑业务类型");
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('删除业务类型', '请选择要删除的业务类型！');
						} else {
							Ext.Msg.show({
								title:'删除业务类型',
								msg: '请再次确实是否要将选择的业务类型删除?',
			...
			buttons: [{
				...
				handler: function() {
					if(form.form.isValid()) {
						if(actiontype == "add") {
							form.form.submit({
								url: window.webRoot + 'Conf/Business',
								method: 'POST',
								params: {
									_method: 'POST'
								},
								success: function(form, action) {
									Ext.Msg.alert('保存成功', '添加业务类型成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('保存失败', '添加业务类型失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
						...
						if(actiontype == "edit") {
							...
							form.form.submit({
								url: window.webRoot + 'Conf/Business/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
								success: function(form, action) {
									Ext.Msg.alert('修改成功', '修改业务类型成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.CONNECT_FAILURE:
											...
											break;
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('修改失败', '修改业务类型失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
