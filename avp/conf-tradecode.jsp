	...
	function doReload() {
		Ext.Msg.show({
			...
			fn: function(btn, text) {
				if(btn == "yes") {
					Ext.Ajax.request({
						url: window.webRoot + 'CodeBase/5009/Reload',
						method: 'GET',
	...
	Ext.onReady(function(){
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'CodeBase/5009'
			})
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>本页面用于显示IVR交易列表。数据以CSR系统的数据库为基础，并由CSR系统维护，本页面只做展示。数据最后一次更新时间为: <span style='color:#0000FF' id=spanReloadTime></span>。<a href=# onclick='doReload()'>手动更新</a></font>"
			})
		);
		...
		store.on('load', function() {
			Ext.Ajax.request({
				url: window.webRoot + 'CodeBase/5009/UpdateTime',
				method: 'GET',
				success: function(response,options) {
					var respText = Ext.util.JSON.decode(response.responseText);
					if(respText.code == 'OK') {
