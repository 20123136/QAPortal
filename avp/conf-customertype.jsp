	...
	Ext.onReady(function(){
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'Confs/Type/CustomerType'
			})
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>本页面用于维护用户类型。'编号'为VP内部定义的客户类型编号，'名称'为客户类型名称。</font>"
			})
		);
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						if(!rec) {
							Ext.Msg.alert('编辑客户类型', '请选择要修改的客户类型！');
						} else {
							actiontype = "edit";
							win.setTitle("编辑客户类型");
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('删除客户类型', '请选择要删除的客户类型！');
						} else {
							Ext.Msg.show({
								title:'删除客户类型',
								msg: '请再次确实是否要将选择的客户类型删除?',
			...
			buttons: [{
				...
				handler: function() {
					if(form.form.isValid()) {
						if(actiontype == "add") {
							form.form.submit({
								url: window.webRoot + 'Conf/Type/CustomerType',
								method: 'POST',
								params: {
									_method: 'POST'
								},
								success: function(form, action) {
									Ext.Msg.alert('保存成功', '添加客户类型成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('保存失败', '添加客户类型失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
						...
						if(actiontype == "edit") {
							...
							form.form.submit({
								url: window.webRoot + 'Conf/Type/CustomerType/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
								success: function(form, action) {
									Ext.Msg.alert('修改成功', '修改客户类型成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.CONNECT_FAILURE:
											...
											break;
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('修改失败', '修改客户类型失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
