	...
	Ext.onReady(function(){
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>本页面用于IVR地点配置。'编号'为VP内部定义的地点编号，'名称'为地点名称。</font>"
			})
		);
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('删除地点', '请选择要删除的地点！');
						} else {
							Ext.Msg.show({
								title:'删除地点',
								msg: '请再次确实是否要将选择的地点删除?',
			...
			buttons: [{
				...
				handler: function() {
					if(form.form.isValid()) {
						if(TransferType == "add") {
							form.form.submit({
								url: window.webRoot + 'Conf/Type/Site',
								method: 'POST',
								params: {
									_method: 'POST'
								},
								success: function(form, action) {
									Ext.Msg.alert('保存成功', '添加地点成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('保存失败', '添加地点失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
						...
						if(TransferType == "edit") {
							...
							form.form.submit({
								url: window.webRoot + 'Conf/Type/Site/' + vals.id,
								method: 'POST',
								params: {
									_method: 'PUT'
								},
								success: function(form, action) {
									Ext.Msg.alert('修改成功', '修改地点成功！');
									store.load();
									win.hide();
								},
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.CONNECT_FAILURE:
											...
											break;
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('修改失败', '修改地点失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
