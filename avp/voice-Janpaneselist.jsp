	...
	Ext.onReady(function(){
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'Voice/JapaneseVoices'
			})
		...
		toolbarHelp.addButton(
			new Ext.form.DisplayField({
				fieldLabel: '',
				style: {align: 'right'},
				html:"&nbsp;&nbsp;<font color='#FF9900'>*</font>&nbsp;&nbsp;<font color='#646464'>本页面用于维护日文语音列表。</font>"
			})
		);
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						...
						if(!rec) {
							Ext.Msg.alert('编辑日文语音', '请选择要修改的日文语音！');
						} else {
							actiontype = "edit";
							win.setTitle("编辑日文语音");
		...
		toolbar.addButton(new Ext.Toolbar.Button({
				...
				listeners: {
					'click': function(obj, e) {
						var rec = grid.getSelectionModel().getSelected();
						if(!rec) {
							Ext.Msg.alert('删除日文语音', '请选择要删除的日文语音！');
						} else {
							Ext.Msg.show({
								title:'删除日文语音',
								msg: '请再次确实是否要将选择的日文语音删除?',
								...
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('保存失败', '添加日文语音失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
								...
								failure: function(form, action) {
									switch (action.failureType) {
										...
										case Ext.form.Action.CONNECT_FAILURE:
											...
											break;
										case Ext.form.Action.SERVER_INVALID:
											Ext.Msg.alert('修改失败', '修改日文语音失败！(' + action.result.message + ')');
									}
								},
								waitMsg: '数据保存中，请稍候...'
							});
