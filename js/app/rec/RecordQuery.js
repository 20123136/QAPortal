	...
	function updateCaseData(rec) {
		Ext.Ajax.request({
			url: window.webRoot + 'QACase/GetByRecord/' + rec.get('id'),
			success: function(res) {
				...
				if(respText.code == 'OK') {
					...
					grid.getStore().on('load', function(obj) {
						var r = obj.getById(rec.id);
						if(!r) {
							win.close();
							playwin.close();
							return;
	...
	function updateButton(rec) {
		...
		if(status == 'None') {
			...
			//Submit: 可以提交，显示按钮‘质检评分’，案例窗口数据清空
			obj.setText('质检评分');
			obj.show();
			obj2.hide();
			obj3.hide();
			...
			//Update: 修改评分，显示按钮‘修改评分’，案例窗口显示当前案例信息，可修改
			obj.setText('修改评分');
			obj.show();
			obj2.hide();
			obj3.hide();
...
*/
	...
	function showCaseWindow(showmode) {
		/** 提交案例 START */		
		var rec = grid.getStore().getById(currentRec.id);
											...
											'click' : function(obj) {
												if(qaWin && qaWin.isVisible()) {
													...
													qaWin = new Ext.app.qa.QASelectorWindow({
														...
														title: obj.getText(),
														types: objGroupTypes,
														groupTypeId: rec.get('qaGroupTypeId'),
														readOnly: true,
														recData: rec,
														iconCls: 'gradebtn',
														ownerObj: grid
