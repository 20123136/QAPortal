	...
	Ext.onReady(function(){
		...
		var store = new Ext.data.JsonStore({
			...
			proxy: new Ext.data.HttpProxy({
				url: window.webRoot + 'Report/QARecord/AgentScoreStat',
				timeout: 300000
			}),
		...
		store.on('load', function(obj, node) {
			...
			var avgScore = stat['avgScore'] && stat['avgScore'] > 0 ? stat['avgScore'] / n : '0.00';
			var times = stat['times'] && stat['times'] > 0 ? stat['times'] / n : '0.00';
			...
			Ext.each(itemGroups, function(group) {
				Ext.each(group.children, function(group2) {
					...
					fields[key] = '<b>' + (stat[key] / n).toFixed(2) + '</b>';
				});
			});
